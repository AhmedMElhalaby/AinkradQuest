...
struct ItemQueryTests {
    ...
    func text() {
        ...
        filter.text = "AUTH"
        #expect(ItemQuery.apply(filter, sort: .manual, to: items, scheme: .softwareDefault).map(\.title) == ["Fix auth refresh"])
    }
    ...
    func prioritySort() {
        ...
        #expect(ItemQuery.apply(ItemFilter(), sort: .priority, to: items, scheme: .softwareDefault).map(\.title)
            == ["urgent", "low"])
    }
    ...
    func dueSort() {
        ...
        #expect(ItemQuery.apply(ItemFilter(), sort: .dueDate, to: items, scheme: .softwareDefault).map(\.title)
            == ["soon", "later", "undated"])
    }
}
